...
// === Data Element (DE) UIDs — Travel Information ===
export const DE_AIRLINE_FLIGHT_COMBINED = "dP5GhQYdMMf"; // e.g., "ET302"
...
// === Data Element (DE) UIDs — Clinical ===
export const DE_CLINICAL_CLASSIFICATION = "cGSuTAbYhkM"; // skip when mapping clinicalData
// Option group for countries visited in the last 21 days
...
function ProgressBar({
  ...
  const colorFor = (idx: number) => {
    if (idx < current) return '#059669'; // completed = green
    if (idx === current) return status === 'error' ? '#dc2626' : '#f59e0b'; // current = red/yellow
    return '#d1d5db'; // future = gray
  };

  return (
    <div className="flex gap-3 mb-6">
      {stages.map((_, idx) => (
        <div key={idx} className="flex-1 h-2 rounded-full" style={{ backgroundColor: colorFor(idx) }} />
      ))}
    </div>
  );
}
...
function PersonalForm(
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <Card>
        <ProgressBar current={0} status={status} />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Field label="Sex" required error={errors.sex}>
            {/* Segmented control */}
            <div
              data-field="sex"                             // helps scrollToFirstError
              role="radiogroup"
              aria-label="Sex"
              className="grid grid-cols-3 bg-gray-100 rounded-2xl overflow-hidden"
            >
              {[
                { label: "Male", value: "MALE" },
                { label: "Female", value: "FEMALE" },
                { label: "-", value: "OTHER" },           // store OTHER, show "-"
              ].map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "py-3 text-lg text-center focus:outline-none focus:ring-2 focus:ring-emerald-600",
                      "transition",
                      selected ? "bg-white font-semibold" : "text-gray-700 hover:bg-white/60",
                    ].join(" ")}
                  >
                    {opt.label}
                  </button>
            ...
            {/* Hidden input so validation/scroll can target name/id="sex" */}
        ...
        <div className="flex justify-between mt-8">
          ...
          {/* Buttons + inline error message */}
          <div className="mt-6 flex items-center justify-between gap-4">
            {/* Left group: error message + Back */}
            <div className="flex items-center gap-3">
              {showError ? (
                <p className="text-red-600 text-sm" role="alert">
                  Some fields are wrong, please correct.
                </p>
              ) : (
                // Reserve space to avoid layout jump when message appears
            ...
            <button
              type="button"
              onClick={next}
              className="px-8 py-3 rounded-2xl bg-emerald-700 text-white text-lg"
            >
              Next
            </button>
          </div>
...
function TravelForm(
  {
    data, errors, update, toggleVisited, back, next, cancel, countriesOptions,
    status = 'in-progress',
    showError = false,
  }: {
    data: any;
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <Card>
        <ProgressBar current={1} status={status} />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Field label="Air Line Name" required error={errors.airline}>
            <select
              id="airlineName" name="airlineName"
              className="w-full rounded-2xl border border-gray-300 px-4 py-3 text-lg"
              value={data.airline || ""}
          ...
          {data.airline === "OTHER" && (
            <Field label="Other airline">
              <input
                id="otherAirline" name="otherAirline"
                className="w-full rounded-2xl border border-gray-300 px-4 py-3 text-lg"
          ...
          <Field label="Departure Country" required error={errors.departureCountry}>
            <SearchableSelect
              id="departureCountry" name="departureCountry"
              options={COUNTRY_LIST}
        ...
        <div className="flex justify-between items-center mt-8">
          ...
          <div className="mt-6 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={back /* omit in PersonalForm if it has no Back */}
                className="px-8 py-3 rounded-2xl bg-gray-200 text-gray-900 text-lg"
              >
                Back
              </button>
            </div>
...
function ClinicalForm(
  {
    data, updateClinical, back, next, cancel, clinicalElements, errors,
    status = 'in-progress',
    showError = false,
  }: {
    data: any;
    updateClinical: (id: string, value: any) => void;
    back: () => void;
    next: () => void;
    cancel: () => void;
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <Card>
        <ProgressBar current={2} status={status} />
...
export default function App() {
  ...
  // Modal to redirect user from blocked submit → Edit Declaration
const [blockNewModal, setBlockNewModal] = useState<{
  open: boolean;
  message: string;
  lastName: string;
  passport: string;
}>({ open: false, message: "", lastName: "", passport: "" });

function openBlockNewModal(opts: { message: string; lastName: string; passport: string }) {
  setBlockNewModal({ open: true, ...opts });
}

function closeBlockNewModal() {
  setBlockNewModal((m) => ({ ...m, open: false }));
}
...
function onBlockContinue() {
  const { lastName, passport } = blockNewModal;
  closeBlockNewModal();
  // Prefill Edit inputs
  setEditTokenInput("");
  setEditLastNameInput(lastName);
  setEditPassportInput(passport);
  // Go to EDIT view
  setView("edit");
  // Focus the Last name box once the view is painted
  requestAnimationFrame(() => {
    const inp = document.getElementById("editLastName") as HTMLInputElement | null;
    inp?.focus();
  });
}
  ...
  // Visual state for the current step: yellow while editing, red on failed Next
  const [progressStatus, setProgressStatus] = useState<'in-progress' | 'error'>('in-progress');
    ...
    async function fetchMetadata() {
      try {
        ...
        const resp = await fetch(
          `${API_PREFIX}/optionGroups/${OG_VISITED_COUNTRIES}?fields=name,options[name,code,sortOrder,translations]`,
        ...
        if (resp2.ok) {
          ...
          // Filter out the Health Status Flag data element (DE_CLINICAL_CLASSIFICATION) from being displayed
          const mapped2 = elems
            .filter((e: any) => e.dataElement.id !== DE_CLINICAL_CLASSIFICATION)
            .sort((a: any, b: any) => a.sortOrder - b.sortOrder)
            .map((e: any) => ({
              id: e.dataElement.id,
  ...
  function validateCurrentStep(): boolean {
    ...
    if (step === 0) {
      ...
      if (!data.lastName?.trim()) e.lastName = "Last name is required.";
        ...
        e.lastName = "Only letters and spaces are allowed.";
      ...
      if (isMinor) {
        ...
        } else if (!/^[\p{L}\s]+$/u.test(data.guardianName.trim())) {
          e.guardianName = "Only letters and spaces";
        }
        const gp = (data.guardianPhone || "").trim();
        if (!gp) {
          e.guardianPhone = "Guardian phone is required";
        } else if (!/^\+?\d{7,15}$/.test(gp)) {
          e.guardianPhone = "Use digits, optional leading + (7–15 digits)";
        }
      }
    }
    if (step === 1) {
    ...
    if (Object.keys(e).length > 0) {
      setProgressStatus('error');     // ensures the bar turns red immediately
      scrollToFirstError(e);          // NEW: robust scroll + focus
    } else {
  ...
  /** ===== Identity sanitation helpers ===== */
  // The goal is to make sure we send normalize text to the server (no leading or trailing spaces, double spaces)
  function normalizeSpaces(s: string) {
    // collapse multiple spaces to one, then trim
    return s.replace(/\s+/g, " ").trim();
  }

  function isISODateInPast(iso?: string | null): boolean {
    if (!iso) return false;
  ...
  function isISODateTodayOrFuture(iso?: string | null) {
    if (!iso) return false;
    const d = new Date(iso);
    if (isNaN(d as any)) return false;
    const today = new Date();
...
// Helper function used to navigate to edit declaration when submission is not possible
// because there is a current submission in progress
function goToEditWithPrefill({
  lastName,
  passport,
  message,
}: {
  ...
  setEditTokenInput("");                // prefer LN + Passport path
  setEditLastNameInput(lastName);
  setEditPassportInput(passport);
  setEditError(message);                // show message in Edit section
  setView("home");                      // go back to home where Edit lives
  requestAnimationFrame(() => {
    const sec = document.getElementById("edit-declaration");
  ...
  // Helpers used to merge attributes of a traveler who is already enrolled
  function toAttrMap(attrs: Array<{ attribute: string; value: any }>) {
    const m = new Map<string, any>();
    (attrs || []).forEach(a => m.set(a.attribute, a.value));
    return m;
  }

  function mergeAttributes(existing: Array<{ attribute: string; value: any }>, outgoing: Array<{ attribute: string; value: any }>) {
    const m = toAttrMap(existing);
    outgoing.forEach(a => m.set(a.attribute, a.value)); // outgoing overrides
    return Array.from(m, ([attribute, value]) => ({ attribute, value }));
  }


  /** Find declaration by:
   *  - declarationId (token)  OR
   *  - lastName + passport
   *
   * Returns { tei, enrollment?, clinicalEvent? } or null.
   */
  async function findDeclaration({
    ...
    lastName?: string;
    passport?: string;
  }): Promise<{ tei: string; enrollment?: string; clinicalEvent?: string } | null> {
    // 1) By declaration token (ID)
    if (declarationId?.trim()) {
      const parts = declarationId.trim().split("-");
      if (parts.length >= 3) {
        return { tei: parts[0], enrollment: parts[1], clinicalEvent: parts[2] };
      }
      throw new Error("Invalid declaration ID format.");
    }

    // 2) By last name + passport
    const ln = normalizeSpaces(lastName || "");
    const pn = (passport || "").trim();
    if (!ln || !pn) {
      throw new Error("Provide Declaration ID OR Last name + Passport.");
    }

    const searchBase =
      `${API_PREFIX}/40/tracker/trackedEntities` +
      `?fields=enrollments[enrollment],trackedEntity,orgUnit` +
      `&program=${PROGRAM_ID}&page=1&ouMode=ACCESSIBLE`;

    // ATT_LAST_NAME = "vUacdogzWI6"; ATT_PASSPORT = "kDWurLVuVZw"
    const url =
      `${searchBase}` +
      `&filter=${ATT_LAST_NAME}:ilike:${encodeURIComponent(ln)}` +
      `&filter=${ATT_PASSPORT}:eq:${encodeURIComponent(pn)}`;
    ...
    const json = await resp.json();
    const inst = (json?.instances && json.instances[0]) || null;
    if (!inst) return null;

    return {
      tei: inst.trackedEntity,
      enrollment: inst.enrollments?.[0]?.enrollment,
    };
  }

  function sanitizeIdentityFields(raw: any) {
    return {
      firstName: raw.firstName?.trim() || "",
      middleName: raw.middleName?.trim() || "",
      // allow Spanish double surname but only ONE space
      lastName: raw.lastName ? normalizeSpaces(raw.lastName) : "",
      // phone: input is restricted to digits/+; just trim
      phone: (raw.phone ?? "").trim(),
      // correct field name is "passport"
      passport: raw.passport?.trim() || "",
      // include guardianName if present (minor)
      guardianName: raw.guardianName ? normalizeSpaces(raw.guardianName) : "",
    };
  }
  function scrollToFirstError(errorsObj: Record<string, string>) {
    const keys = Object.keys(errorsObj);
    if (!keys.length) return;
    const firstKey = keys[0];

    // Try several selectors to find the field or its wrapper
    const el =
    ...
    // If you have a sticky header/progress (~80–140px), offset a bit
    const y = (el as HTMLElement).getBoundingClientRect().top + window.scrollY - 120;

    window.scrollTo({ top: y, behavior: 'smooth' });

    // Try focusing the element for accessibility & visibility
    (el as HTMLElement).focus?.();
  }

  function BlockNewDeclarationModal({
  open,
  message,
  onCancel,
  onContinue,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-[min(36rem,92vw)] rounded-2xl bg-white shadow-xl p-6">
        <h2 className="text-xl font-semibold mb-2">Existing declaration detected</h2>
        <p className="text-gray-700">{message}</p>
        <div className="mt-6 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="px-5 py-3 rounded-2xl bg-rose-600 text-white text-base font-semibold hover:bg-rose-700"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onContinue}
            className="px-5 py-3 rounded-2xl bg-blue-600 text-white text-base font-semibold hover:bg-blue-700"
          >
            Continue to edit declaration
          </button>
        </div>
      </div>
    </div>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editing) {
        ...
        // 0) Sanitize once up front
        const ident = sanitizeIdentityFields(data); // trims, single-space lastName, etc.
        const dobIso = (data.dob || "").trim();

        // 1) Build a search base that ALREADY includes attributes (no extra fetch later)
        const searchBase =
          `${API_PREFIX}/40/tracker/trackedEntities` +
          `?fields=enrollments[enrollment,enrolledAt],trackedEntity,orgUnit,attributes[attribute,value]` +
          `&program=${PROGRAM_ID}&page=1&ouMode=ACCESSIBLE`;

        const auth = { headers: { Authorization: `ApiToken ${DHIS2_TOKEN}` } };

        let found: any = null;

        // ---------- Scenario A: Passport + Nationality ----------
        if (!found && ident.passport && data.nationality) {
          const url =
            `${searchBase}` +
            `&filter=${ATT_PASSPORT}:eq:${encodeURIComponent(ident.passport)}` +
            `&filter=${ATT_NATIONALITY}:eq:${encodeURIComponent(data.nationality)}`;

          const resp = await fetch(url, auth);
          if (resp.ok) {
            const json = await resp.json();
            const candidates = json?.instances || [];
            if (candidates.length > 0) {
              // Prefer strict exact match on both attrs if multiple rows
              const exact = candidates.find((tei: any) => {
                const m = new Map(tei.attributes.map((a: any) => [a.attribute, a.value]));
                return String(m.get(ATT_PASSPORT)).trim() === ident.passport &&
                  String(m.get(ATT_NATIONALITY)) === data.nationality;
              });
              found = exact || candidates[0];
            }
          }
        }

        // ---------- Scenario B (fallback): First + Last + DOB ----------
        if (!found && ident.firstName && ident.lastName && dobIso) {
          // Use ilike for names to be resilient; filter DOB by exact equality
          let url =
            `${searchBase}` +
            `&filter=${ATT_FIRST_NAME}:ilike:${encodeURIComponent(ident.firstName)}` +
            `&filter=${ATT_LAST_NAME}:ilike:${encodeURIComponent(ident.lastName)}` +
            `&filter=${ATT_DATE_OF_BIRTH}:eq:${encodeURIComponent(dobIso)}`;

          // Optional: if you want to further narrow when middle name is present
          // (doesn't change the rule, just helps disambiguate similar records)
          if (ident.middleName) {
            url += `&filter=${ATT_MIDDLE_NAME}:ilike:${encodeURIComponent(ident.middleName)}`;
          }
          ...
          if (resp.ok) {
            ...
            const candidates = json?.instances || [];
            if (candidates.length > 0) {
              // Prefer strict exact match after normalizing spaces (case-insensitive)
              const norm = (s: string) => s.replace(/\s+/g, " ").trim().toLowerCase();
              const exact = candidates.find((tei: any) => {
                const m = new Map(tei.attributes.map((a: any) => [a.attribute, a.value]));
                return (
                  norm(String(m.get(ATT_FIRST_NAME) || "")) === norm(ident.firstName) &&
                  norm(String(m.get(ATT_LAST_NAME) || "")) === norm(ident.lastName) &&
                  String(m.get(ATT_DATE_OF_BIRTH) || "") === dobIso
                );
              });
              found = exact || candidates[0];
            }
        ...
        if (found) {
          ...
          // ===== BLOCK NEW SUBMISSION IF A CURRENT/FUTURE TRAVEL EVENT EXISTS =====
          // Fetch the TEI with events so we can inspect the latest Travel Information event.
          const respEv = await fetch(
            `${API_PREFIX}/40/tracker/trackedEntities/${teiUid}` +
            `?program=${PROGRAM_ID}` +
            `&fields=enrollments[events[event,programStage,occurredAt,completedAt]]`,
            { headers: { Authorization: `ApiToken ${DHIS2_TOKEN}` } }
          );
          if (!respEv.ok) {
            throw new Error(`Failed to check existing declaration events (${respEv.status}).`);
          }
          const fullEv = await respEv.json();

          // Find the most recent Travel Information event (by occurredAt).
          let latestTravelISO: string | undefined;
          (fullEv.enrollments || []).forEach((enr: any) => {
            (enr.events || []).forEach((ev: any) => {
              if (ev.programStage === TRAVEL_STAGE_ID) {
                const iso = ev.occurredAt || ev.completedAt || null;
                if (iso) {
                  if (!latestTravelISO || new Date(iso) > new Date(latestTravelISO)) {
                    latestTravelISO = iso;
                  }
                }
              }
          ...
          if (latestTravelISO) {
            const dateOnly = String(latestTravelISO).includes("T")
              ? String(latestTravelISO).split("T")[0]
              : String(latestTravelISO).slice(0, 10);

if (isISODateTodayOrFuture(dateOnly)) {
  openBlockNewModal({
    lastName: ident.lastName,
    passport: ident.passport,
    message:
      "There is a current declaration submitted, so no new submissions are allowed. You can continue to the Edit Declaration page to modify your existing submission.",
  });
  return; // do not proceed with submit
      ...
      // Attributes mapping
      // Sanitize just-in-time
      const ident = sanitizeIdentityFields(data);
      ...
      // === Build attributes ===
      pushAttr(ATT_FIRST_NAME, ident.firstName);
      pushAttr(ATT_MIDDLE_NAME, ident.middleName);
      pushAttr(ATT_LAST_NAME, ident.lastName);
      pushAttr(ATT_DATE_OF_BIRTH, data.dob);
      pushAttr(ATT_SEX, data.sex);
      pushAttr(ATT_PHONE, ident.phone);
      pushAttr(ATT_NATIONALITY, data.nationality);
      pushAttr(ATT_PASSPORT, ident.passport);

      // Guardian fields (only if present)
      if (ident.guardianName) {
        pushAttr(ATT_GUARDIAN_FULL_NAME, ident.guardianName);
      }
      if (data.guardianPhone) {
        pushAttr(ATT_GUARDIAN_PHONE, data.guardianPhone);
      }

      // ==== Merge with existing attributes (resident/region/zone/woreda/kebele/etc.) ====
      const existingAttrs = found?.attributes ?? []; // <-- came in same response via fields=...attributes...
      const finalAttributes = mergeAttributes(existingAttrs, attributes);
      ...
      // Optional: ensure the combined code uses the 2-letter airline code
      const airlineCode = data.airline === "OTHER" ? (data.otherAirline || "") : data.airline;

      travelValues.push({ dataElement: DE_AIRLINE_FLIGHT_COMBINED, value: `${airlineCode}${data.flightNumber}` });
      travelValues.push({ dataElement: DE_PURPOSE, value: data.purpose });
      travelValues.push({ dataElement: DE_AIRLINE_NAME, value: data.airline });
      if (data.airline === "OTHER" && data.otherAirline) {
        travelValues.push({ dataElement: DE_OTHER_AIRLINE, value: data.otherAirline });
      }
      travelValues.push({ dataElement: DE_FLIGHT_NUMBER, value: data.flightNumber });
      travelValues.push({ dataElement: DE_SEAT_NUMBER, value: data.seatNumber });
      travelValues.push({ dataElement: DE_DEPARTURE_COUNTRY, value: data.departureCountry });
      travelValues.push({ dataElement: DE_DEPARTURE_CITY, value: data.departureCity });

      // Place of Stay is no longer collected; do not include FoXx0EfvELo

      const visitedKeys = [DE_VISITED_1, DE_VISITED_2, DE_VISITED_3, DE_VISITED_4, DE_VISITED_5];
      ...
      //clinicalValues.push({ dataElement: "DE_CLINICAL_CLASSIFICATION", value: "null" });
      const clinicalEvent = {
        status: "COMPLETED",
        completedAt: occurredAt,
        occurredAt: occurredAt,
      ...
      if (!postResp.ok) {
        ...
        for (const ev of enrollmentData.events || []) {
          for (const dv of ev.dataValues || []) {
            if (dv.dataElement === DE_CLINICAL_CLASSIFICATION) {
  ...
  async function handleEditLookup() {
    ...
    const declId = (editTokenInput || "").trim();
    const ln = normalizeSpaces(editLastNameInput || "");
    const pn = (editPassportInput || "").trim();

    if (!declId && !(ln && pn)) {
      setEditError("Please enter a Declaration ID OR Last name + Passport.");
      return;
    }
    ...
    try {
      ...
      if (declId) {
        // Parse token: <tei>-<enrollment>-<clinicalEvent>
        const parts = declId.split("-");
        if (parts.length >= 3) {
          tei = parts[0];
          enrollment = parts[1];
          clinicalEvent = parts[2];
        } else {
          throw new Error("Invalid Declaration ID format. Use the full token, e.g., <tei>-<enrollment>-<event>.");
        }
      } else {
        // Lookup by Last name (ilike) + Passport (eq)
        const searchBase =
          `${API_PREFIX}/40/tracker/trackedEntities` +
          `?fields=enrollments[enrollment],trackedEntity,orgUnit` +
          `&program=${PROGRAM_ID}&page=1&ouMode=ACCESSIBLE`;
        ...
        const json = await resp.json();
        const found = (json?.instances && json.instances[0]) || null;
        if (!found) {
          throw new Error("No declaration found for the provided Last name + Passport.");
        }

        tei = found.trackedEntity;
        if (found.enrollments && found.enrollments.length > 0) {
          enrollment = found.enrollments[0].enrollment;
        }
      }

      if (!tei) throw new Error("Unable to determine the traveler (TEI).");

      // Fetch full TEI (attributes + events)
      const resp = await fetch(
        `${API_PREFIX}/40/tracker/trackedEntities/${tei}` +
        `?program=${PROGRAM_ID}` +
      ...
      if (!full.enrollments || full.enrollments.length === 0) {
        throw new Error("No enrollment found for this traveler.");
      }

      const enrollmentObj = enrollment
        ? full.enrollments.find((e: any) => e.enrollment === enrollment) || full.enrollments[0]
        : full.enrollments[0];
      ...
      if (full.attributes) {
        full.attributes.forEach((att: any) => {
          attrMap[att.attribute] = att.value;
        });
      }

      const newData: any = { ...data };
      newData.firstName = attrMap[ATT_FIRST_NAME] || "";
      newData.middleName = attrMap[ATT_MIDDLE_NAME] || "";
      newData.lastName = attrMap[ATT_LAST_NAME] || "";
      newData.dob = attrMap[ATT_DATE_OF_BIRTH] || "";
      newData.sex = attrMap[ATT_SEX] || "";
      newData.phone = attrMap[ATT_PHONE] || "";
      newData.nationality = attrMap[ATT_NATIONALITY] || "";
      newData.passport = attrMap[ATT_PASSPORT] || "";
      newData.guardianName = attrMap[ATT_GUARDIAN_FULL_NAME] || newData.guardianName || "";
      newData.guardianPhone = attrMap[ATT_GUARDIAN_PHONE] || newData.guardianPhone || "";
      ...
      // Pick latest events for Travel / Clinical stages
      ...
      // If there is a Travel event, hydrate AND enforce "no past arrival" rule
      if (travelFound) {
        travelEvent = travelFound.event;

        // Arrival date (occurredAt) → YYYY-MM-DD
        let arrivalIso: string | undefined;
        if (travelFound.occurredAt) {
          const s = String(travelFound.occurredAt);
          arrivalIso = s.includes("T") ? s.split("T")[0] : s.slice(0, 10);
          newData.arrivalDate = arrivalIso;
        }

        // BLOCK EDIT if arrival date is already in the past
        if (isISODateInPast(arrivalIso)) {
          throw new Error("There is no current declaration in progress to edit.");
        }

        // Map Travel DEs using constants
        (travelFound.dataValues || []).forEach((dv: any) => {
          const de = dv.dataElement;
          const val = dv.value;
          switch (de) {
            case DE_AIRLINE_FLIGHT_COMBINED: {
            ...
            case DE_PURPOSE: newData.purpose = val; break;
            case DE_AIRLINE_NAME: if (!newData.airline) newData.airline = val; break;
            case DE_OTHER_AIRLINE: newData.otherAirline = val; break;
            case DE_FLIGHT_NUMBER: newData.flightNumber = val; break;
            case DE_SEAT_NUMBER: newData.seatNumber = val; break;
            case DE_DEPARTURE_COUNTRY: newData.departureCountry = val; break;
            case DE_DEPARTURE_CITY: newData.departureCity = val; break;

            // Visited countries — map name → ISO code for the UI
            case DE_VISITED_1:
            case DE_VISITED_2:
            case DE_VISITED_3:
            case DE_VISITED_4:
      ...
      if (clinicalFound) {
        ...
        (clinicalFound.dataValues || []).forEach((dv: any) => {
          const de = dv.dataElement;
          const val = dv.value;
          if (de !== DE_CLINICAL_CLASSIFICATION) {
      ...
      const msg =
        err?.message ||
        "Lookup failed. Please enter a Declaration ID OR Last name + Passport.";
      setEditError(msg);
    } finally {
    ...
    try {
      ...
      if (latestClinical.dataValues) {
        latestClinical.dataValues.forEach((dv: any) => {
          if (dv.dataElement === DE_CLINICAL_CLASSIFICATION) classification = dv.value || classification;
        });
      }
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <Card>
              <div className="space-y-4">
                ...
                <p className="text-gray-600">Find an existing submission by Declaration ID or Last name / Passport Number.</p>
                <button
        ...
        <PersonalForm
          ...
          next={() => {
            if (validateCurrentStep()) {
              setStep(1);
              setProgressStatus('in-progress'); // next step starts yellow
            } else {
              setProgressStatus('error');       // current step turns red
            }
          }}
          status={progressStatus}
          showError={progressStatus === 'error'}
        />
      );
    }
    ...
    if (step === 1) {
      return (
        <TravelForm
          ...
          back={() => { setStep(0); setProgressStatus('in-progress'); }}
          cancel={cancelDeclaration}
          next={() => {
            if (validateCurrentStep()) {
              setStep(2);
    ...
    if (step === 2) {
      return (
        <ClinicalForm
          data={data}
          errors={errors}
          updateClinical={updateClinical}
          back={() => { setStep(1); setProgressStatus('in-progress'); }}
          cancel={cancelDeclaration}
          next={() => {
            if (validateCurrentStep()) {
              setStep(3);
...
if (step === 3) return (
  <>
    <SummaryView
      ...
      setAgreeLegal={setAgreeLegal}
    />
    <BlockNewDeclarationModal
      open={blockNewModal.open}
      message={blockNewModal.message}
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-5xl mx-auto p-4">
          <Card>
            {/* EDIT DECLARATION SECTION */}
            <section id="edit-declaration">
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  {qrClassification && (
                    <p className="text-lg font-semibold mt-2">
                      Risk Classification: <span className={qrClassification === "RED" ? "text-red-600" : qrClassification === "YELLOW" ? "text-yellow-500" : "text-emerald-600"}>{qrClassification}</span>
                    </p>
                  )}
                </div>
              </div>
